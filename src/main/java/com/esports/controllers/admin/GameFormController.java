...
public class GameFormController {
    ...
    private boolean validateInputs() {
        ...
        if (name.length() < 2) { 
            showAlert("Game name must be at least 2 characters!"); 
            return false; 
        }
        if (name.length() > 100) { 
            showAlert("Game name must not exceed 100 characters!"); 
            return false; 
        }
        ...
        if (slug.length() < 2) { 
            showAlert("Slug must be at least 2 characters!"); 
            return false; 
        }
        if (slug.length() > 100) { 
            showAlert("Slug must not exceed 100 characters!"); 
            return false; 
        }
        if (!slug.matches("^[a-z0-9-]+$")) { 
            showAlert("Slug can only contain lowercase letters, numbers and hyphens!"); 
            return false; 
        }
            ...
            if (!backgroundImage.isEmpty()) {
                javafx.application.Platform.runLater(() -> {
                    try {
                        ...
                        showInfo("[SUCCESS] Game fetched from RAWG!");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                });
